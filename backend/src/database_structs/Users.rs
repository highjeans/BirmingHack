...
use uuid::Uuid;
use crate::{routes::user_routes::Claims, schema::{self, users::dsl as user_table}};
...
impl<'r> FromRequest<'r> for Users {
    ...
    async fn from_request(req: &'r Request<'_>) -> request::Outcome<Self, Self::Error> {
        if let Some(jwt) = req.headers().get("Authorization").next() {
            ...
            let data = match decode::<Claims>(jwt, &DecodingKey::from_secret(std::env::var("JWT_KEY").unwrap().as_bytes()), &validation) {
                Ok(c) => c,
                Err(_) => { return request::Outcome::Error((Status::Unauthorized, "Invalid JWT".to_string())) },
            };
            ...
            if let request::Outcome::Error(err) = db_guard {
                return request::Outcome::Error((err.0, "Unable to connect to database".to_string()));
            }
