...
pub fn produce_embeddings(s: &str) -> Option<Vec<f32>> {
    let model = SentenceEmbeddingsBuilder::remote(SentenceEmbeddingsModelType::AllMiniLmL12V2)
        .create_model()
        .ok()?;
    let output = model.encode(&[s]).ok()?;
    Some(output.first()?.to_vec())
}

pub fn cosine_similarity(v: &Vec<f32>, u: &Vec<f32>) -> f32 {
    let dp = dot_prod(v, u);
...
mod tests {
    ...
    fn it_works() {
        ...
        let output: Vec<Vec<f32>> = sentences
            .into_iter()
            .filter_map(produce_embeddings)
            .collect();
        let sim = cosine_similarity(&output[0], &output[1]);
        let disim = cosine_similarity(&output[1], &output[2]);
        assert!(sim > disim)
    }
}
